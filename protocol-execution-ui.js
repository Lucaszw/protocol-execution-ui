...
class Dummy extends MQTTClient {
  constructor() {
    super();
  }
  listen() {
    this.onTriggerMsg("execute", this.execute.bind(this));
    this.bindTriggerMsg("step-model", "register", "register");
    this.bindSignalMsg("complete", "complete");
    this.onSignalMsg("step-model", "register-plugins", this.register.bind(this));
  }
  register() {
    console.log("DUMMY", "REGISTERING!!!!");
    this.trigger("register", {name: this.name});
  }
  execute() {
...
class ProtocolExecutionUI extends UIPlugin {
  ...
  async play() {
    ...
    while (stepNumber < steps.length) {
      ...
      await microdrop.electrodes.putChannels(channels);
